...
The formulation is solved in two phases: a deterministic
mixed-integer program that minimizes reliability intervention costs
over the gating window, followed by a second-order cone program that
verifies worst-case dispatch feasibility under ellipsoidal
uncertainty.
When robust verification fails, column-and-constraint generation
iterates between the phases to close the gap.
...
create uplift charges~\cite{ferc2020order2222}.
...
pioneered by Bertsimas et~al.~\cite{bertsimas2013adaptive}, who
introduced a polyhedral uncertainty set for net load and solved
the two-stage problem via Benders decomposition.
Subsequent work by Jiang et~al.~\cite{jiang2012robust} developed
...
performance.
Zhao and Guan~\cite{zhao2013unified} extended the framework to
...
Papavasiliou et~al.~\cite{papavasiliou2013multiarea} demonstrated the
value of stochastic optimization for wind integration, and
Zheng et~al.~\cite{zheng2013decomposition} developed decomposition methods
...
by Xiong et~al.~\cite{xiong2017distributionally} and others.

Linear decision rules (LDR) provide a tractable approximation of
the recourse policy in robust and adaptive optimization.
Ben-Tal et~al.~\cite{bental2004adjustable} established the affine
adjustable robust counterpart framework, and Lorca and
Sun~\cite{lorca2016multistage, lorca2017robust} applied LDR to unit commitment with
...
prices~\cite{hogan2003minimum, oneill2005efficient}.
The convex hull pricing literature~\cite{gribik2007market, schiro2016convex} has established that pricing in the presence of
...
Andrianesis et~al.~\cite{andrianesis2022computation} quantify the
...
Ela et~al.~\cite{ela2011operating} analyze the operational and market
...
and finite recourse~\cite{zeng2013solving}.
